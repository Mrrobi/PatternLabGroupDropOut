...
def split_data(SOURCE, TRAINING, TESTING, SPLIT_SIZE):
    ...
    for fname in all_list:
        ...
        if ( os.path.getsize(fp) > 0 ):
            ...
            data_path.append(fname)
        
        else:
            print('ignore file')
    
    train_split = int (len(data_path) * SPLIT_SIZE)
    
    list_shuffle = random.sample(data_path,len(data_path))
    
    train_data = list_shuffle [:train_split]
    
    test_data = list_shuffle [train_split:]
